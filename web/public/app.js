...
let currentAccess = { canReadPrivate: false, canWrite: false, canAdmin: false };
let editingDocument = null;
let isDirty = false;

// Helpers for tier-aware UI. `currentUser.kind` is 'owner' for OAuth sessions,
// 'viewer' for read-only passcode sessions, 'editor' for write passcode sessions.
function isDemoSession() {
  return !!currentUser && (currentUser.kind === 'viewer' || currentUser.kind === 'editor');
}
function canWriteInUI() {
  return !!currentUser && currentAccess.canWrite;
}
...
async function showDocument(docPath) {
  ...
  // Show edit button if user can write AND document is editable (not auto-generated).
  // Demo viewer sessions are authenticated but cannot write, so the gate is canWriteInUI.
  const editBtn = document.getElementById('edit-btn');
  if (editBtn) {
    const isAutoGenerated = doc.tags && doc.tags.includes('auto-generated');
    editBtn.style.display = canWriteInUI() && !isAutoGenerated ? 'inline-block' : 'none';
...
async function checkAuth() {
  try {
    ...
    if (response.ok) {
      ...
      // The /user endpoint now returns an explicit access struct. Fall back to
      // owner-style defaults for older deploys where the field is absent.
      currentAccess = data.access || {
        canReadPrivate: !!currentUser,
        canWrite: !!currentUser,
        canAdmin: !!currentUser,
      };
    } else {
      currentUser = null;
      currentAccess = { canReadPrivate: false, canWrite: false, canAdmin: false };
    }
  } catch (err) {
...
function updateAuthUI() {
  ...
  const demoBannerLabel = document.getElementById('demo-banner-label');
  ...
  if (currentUser) {
    ...
    if (userNav) {
      ...
      // Demo sessions have no avatar / display name — substitute a tier label.
      if (userAvatar) {
        userAvatar.src = currentUser.avatar_url || '';
        userAvatar.style.display = currentUser.avatar_url ? '' : 'none';
      }
      if (userName) {
        if (isDemoSession()) {
          userName.textContent =
            currentUser.kind === 'viewer' ? 'Demo (viewer)' : 'Demo (editor)';
        } else {
    ...
    if (logoutBtn) {
      ...
      logoutBtn.textContent = isDemoSession() ? 'Exit demo' : 'Logout';
    }
    // Edit / New affordances depend on canWrite, not just presence of a session.
    ...
    if (newDocBtn) {
      newDocBtn.style.display = canWriteInUI() ? '' : 'none';
    }
    // Demo banner: visible only for passcode-based sessions.
    if (demoBanner) {
      if (isDemoSession()) {
        demoBanner.style.display = '';
        if (demoBannerLabel) {
          demoBannerLabel.textContent =
            currentUser.kind === 'viewer'
              ? 'Demo session — read-only view'
              : 'Demo session — editor (limited writes)';
        }
      } else {
    ...
    // Show login link, hide user nav and demo banner
    if (loginNavLink) loginNavLink.style.display = 'inline';
    if (userNav) userNav.style.display = 'none';
    if (editBtn) editBtn.style.display = 'none';
    if (newDocBtn) newDocBtn.style.display = 'none';
...
function setupEditor() {
  ...
  // Demo banner "Exit demo" button — same code path as Logout, just relabeled.
  const demoBannerExit = document.getElementById('demo-banner-exit');
  if (demoBannerExit) {
    demoBannerExit.addEventListener('click', handleLogout);
  }
...
async function saveDocument() {
  if (!canWriteInUI()) {
    alert(
      currentUser && currentUser.kind === 'viewer'
        ? 'This is a read-only demo session. Sign in with GitHub to make changes.'
...
async function refreshQuickView() {
  ...
  // Trigger a full index rebuild via GitHub Actions. Server-side rebuild-index
  // requires admin (owner) privileges; viewer/editor demo sessions skip the
  // backend trigger and just refresh the frontend cache.
  let rebuildTriggered = false;
  if (currentUser && currentAccess.canAdmin) {
    try {
      ...
      <tr>
        ...
        <td class="api-inv-notes">
          <div class="api-inv-notes-cell"
               contenteditable="${canWriteInUI() ? 'true' : 'false'}"
               data-term-id="${svc.term}"
               data-placeholder="Add notes..."
               ${!canWriteInUI() ? 'style="cursor:default"' : ''}
          >${notes}</div>
        </td>
      </tr>
    `;
