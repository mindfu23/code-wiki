...
async function inferFromManifests(
  ...
  for (const hit of manifests) {
    ...
    // Dependency-based signal (package.json / pubspec.yaml / requirements.txt)
    ...
    } else if (hit.kind === 'requirements.txt') {
      // Python deps. Format: `package_name`, optionally with version specifier
      // (==1.0, >=2.0, ~=1.2), optionally extras (`pkg[extra1,extra2]`).
      // Comments after `#`. Skip `-r other.txt`, `-e .`, URLs, blank lines.
      const raw = await fs.readFile(hit.absPath, 'utf-8');
      for (const line of raw.split('\n')) {
        const stripped = line.split('#')[0].trim();
        if (!stripped) continue;
        if (stripped.startsWith('-')) continue;        // -r, -e, --extra-index, etc.
        if (/^https?:|^git\+/.test(stripped)) continue; // URL-based deps
        // Match leading package name (letters/digits/-_.) before extras / version specifier
    ...
    // pubspec.yaml and pyproject.toml dep parsing is intentionally light —
    // their MANIFEST_SIGNALs already set dart-flutter and python respectively.
    // Deep parsing of these formats requires a YAML/TOML library and is
    // deferred until concrete cases motivate it.
  }
...
async function inferProjectTaxonomy(
  ...
  // Default visibility: prefer the GitHub-derived visibility signal from
  // either source (index-full.json or repo-locations.md). Otherwise, map
  // status: local-only → internal, else public.
  let visibility = 'public';
  const ghVis = entry.visibility ?? entry.ghVisibility;
  if (ghVis === 'private') visibility = 'private';
...
/** Drop keys with `undefined` values so js-yaml doesn't choke on them. */
...
function stubContent(repoName: string, inferred: InferredTaxonomy): string {
  const fm: Record<string, unknown> = {
    ...
    taxonomy: { ...stripUndefined(inferred as Record<string, unknown>), completionState: 'stub' },
  };
